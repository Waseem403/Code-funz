...
import {
  OtherUserReactloveToFeedReply,
  OtherUserReactUnloveToFeedReply,
  OtherUserReactlikeToFeedReply,
  OtherUserReactDislikeToFeedReply
...
class PostReplies extends Component {
  ...
  onLoveClick = Love_Info => {
    this.props.OtherUserReactloveToFeedReply(Love_Info)
  }

  //function which dislove the post
  onUnloveClick = Love_Info => {
    console.log(Love_Info)
    this.props.OtherUserReactUnloveToFeedReply(Love_Info)
  }

  //function which like the post
  onlikeClick = like_Info => {
    this.props.OtherUserReactlikeToFeedReply(like_Info)
  }

  //function which dislike the post
  onUnlikeClick = like_Info => {
    this.props.OtherUserReactDislikeToFeedReply(like_Info)
  }
    ...
    return (
      <React.Fragment key={FeedReply._id}>
        <List className={classes.root} style={{ paddingLeft: "4%" }}>
          ...
          <ReplyMenu anchorRl={anchorRl} Post_Id={Post_Id} CurrentPostProfileId={CurrentTargetProfile} Reply={FeedReply} handleClose={this.handleClose} CurrentReplyUser={FeedReply.user} PostFeedAuthUser={PostFeedAuthUser} />
